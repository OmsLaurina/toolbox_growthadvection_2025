...
save('outputs/positions_AB.mat', 'all_val');
...
regionB = histcn([all_val.Longitude all_val.Latitude], lon_edges, lat_edges, 'AccumData', all_val.Region==2);

% Proportions
denom = regionA + regionB;
output2D.percentA = regionA ./ denom;
output2D.percentB = regionB ./ denom;

% Add flag for A, F and B
Region_flag = nan(size(output2D.percentA));
Region_flag(output2D.percentA == 1) = 1; % A
Region_flag(output2D.percentB == 1) = 2; % B
Region_flag(isnan(Region_flag) & ~isnan(output2D.percentA)) = 3; % F
output2D.Region_flag = Region_flag;

save('outputs/output2D_AB.mat', 'output2D');
...
colorbar('Ticks', [1 2], 'TickLabels', {'A','B'});
title('Trajectories');
print('-djpeg','-r300','figures/adv_AB_traj.jpg')
...
shading flat;
cmap2=colormap([0.6 0.8 1.0; 0.6 0.9 0.6; 1.0 0.6 0.6]);
colorbar('Ticks', [1 2 3], 'TickLabels', {'A','B','F'});
title('6 mai - Label');
xlabel('Longitude');
ylabel('Latitude');
hold on;
scatter(tsg_transect.Longitude, tsg_transect.Latitude, 5, colors, 'filled');
scatter(tsg_stations.Longitude, tsg_stations.Latitude, 5, colors_stations, 'filled');
print('-djpeg','-r300','figures/adv_label_AB.jpg');

% 4. Position of the particle for the target date
figure;
...
print('-djpeg','-r300','figures/adv_AB_finalscatter_label.jpg');