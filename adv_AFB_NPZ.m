...
for t0 = t0_range
    ...
    for day = 0:nb_days_advec-1
        ...
        for substep = 1:(1/dt)
            ...
            Gmax1_tmp = Gmax1_A * (adt_day < adt_threshold_A) + ...
                    Gmax1_B * (adt_day > adt_threshold_B) + ...
                    Gmax1_F * (adt_day >= adt_threshold_A & adt_day <= adt_threshold_B);
            Gmax1(:, :, t_idx) = Gmax1_A .* lat_mask + Gmax1_tmp .* (~lat_mask);

            Gmax2_tmp = Gmax2_A * (adt_day < adt_threshold_A) + ...
                    Gmax2_B * (adt_day > adt_threshold_B) + ...
                    Gmax2_F * (adt_day >= adt_threshold_A & adt_day <= adt_threshold_B);
            Gmax2(:, :, t_idx) = Gmax2_A .* lat_mask + Gmax2_tmp .* (~lat_mask);
        end
    end
